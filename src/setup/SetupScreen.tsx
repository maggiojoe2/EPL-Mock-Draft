...
function updateTeam(teams: Team[], index: number, mapper: (t: Team) => Team): Team[] {
...
export default function SetupScreen({ onDraftStart }: SetupScreenProps) {
  ...
  const handlePlayerPoolFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (players.length === 0) {
        setImportError('Player pool CSV produced no valid rows. Check column names: name, position, nfl_team, adp')
        return
      }
  ...
  const handleRosterFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    try {
      const text = await readFileAsText(file)
  ...
  const removePlayerFromRoster = useCallback((teamIndex: number, playerId: string) => {
    setTeams(prev => updateTeam(prev, teamIndex, team => {
      const previousYearRoster = team.previousYearRoster.filter(p => p.id !== playerId)
      ...
      return { ...team, previousYearRoster, franchiseEligibleIds, saveHistory, franchisePlayer }
    }))
  }, [])

  const toggleFranchiseEligible = useCallback((teamIndex: number, playerId: string) => {
    setTeams(prev => updateTeam(prev, teamIndex, team => {
      const franchiseEligibleIds = new Set(team.franchiseEligibleIds)
  ...
  const togglePreviouslySaved = useCallback((teamIndex: number, playerId: string) => {
    setTeams(prev => updateTeam(prev, teamIndex, team => {
      const saveHistory = new Set(team.saveHistory)
  ...
  const setFranchisePlayer = useCallback((teamIndex: number, playerId: string) => {
    setTeams(prev => updateTeam(prev, teamIndex, team => {
      const player = team.previousYearRoster.find(p => p.id === playerId) ?? null
  ...
  const userEligiblePlayers = userTeam
    ? userTeam.previousYearRoster.filter(p => userTeam.franchiseEligibleIds.has(p.id))
    : []

  const validationErrors: string[] = []
  if (playerPool.length === 0) validationErrors.push('Import a player pool CSV first.')
  if (teams.length < 2) validationErrors.push('Import team rosters CSV (need at least 2 teams).')
      ...
      <section className="setup-section">
        ...
        <div className="import-row">
          <label className="file-label">
            ...
            <input type="file" accept=".csv" onChange={handlePlayerPoolFile} />
            <span className="file-status">
              {playerPool.length > 0 ? `✓ ${playerPool.length} players` : 'No file selected'}
            </span>
          </label>
          <label className="file-label">
        ...
        <div className="csv-hint">
          <details>
            ...
            <p>
              <strong>Player pool:</strong> <code>name, position, nfl_team, adp</code>
            </p>
            <p>
              <strong>Rosters:</strong>{' '}
              <code>team_name, player_name, franchise_eligible, previously_saved</code>
            </p>
          </details>
        </div>
      </section>
        ...
        <>
          ...
          <section className="setup-section">
            ...
            <div className="mode-toggle">
              <label className={mode === 'practice' ? 'mode-opt selected' : 'mode-opt'}>
                <input
                  type="radio"
                  name="mode"
                  value="practice"
            ...
            {mode === 'practice' && (
              <div className="team-select-row">
                ...
                <select
                  id="team-select"
                  value={userTeamIndex ?? ''}
                  onChange={e => setUserTeamIndex(e.target.value === '' ? null : Number(e.target.value))}
                >
          ...
          {franchiseStepVisible && (
            <section className="setup-section">
              ...
              {userEligiblePlayers.length === 0 ? (
                ...
                <div className="team-select-row">
                  <label htmlFor="franchise-select">Declare your franchise player:</label>
                  <select
                    id="franchise-select"
                    value={userTeam!.franchisePlayer?.id ?? ''}
                    onChange={e => setFranchisePlayer(userTeamIndex, e.target.value)}
                  >
                    <option value="">— select —</option>
                            ...
                            <tr key={player.id}>
                              ...
                              <td className="center">
                                <input
                                  ...
                                  onChange={() => toggleFranchiseEligible(ti, player.id)}
                                />
                              </td>
                              <td className="center">
                                <input
                                  ...
                                  onChange={() => togglePreviouslySaved(ti, player.id)}
                                />
                              </td>
                              <td>
                                <button
                                  className="remove-btn"
                                  onClick={() => removePlayerFromRoster(ti, player.id)}
                            ...
                            <ul className="player-search-results">
                              {filteredPool.map(p => (
                                <li key={p.id}>
                                  ...
                                  >
                                    {p.name} ({p.position}, {p.nflTeam}) — ADP #{p.adp}
                                  </button>
                                </li>
                              ))}
                            </ul>
