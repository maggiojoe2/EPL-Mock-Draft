...
describe('buildTeamsFromImport', () => {
  it('creates one Team per entry in the roster import', () => {
    const pool: Player[] = [makePlayer('Josh Allen', 'QB', 1), makePlayer('Saquon Barkley', 'RB', 2)]
    const roster: RosterImport = new Map([
      ['Team A', [{ playerName: 'Josh Allen', franchiseEligible: false, previouslySaved: false }]],
      ['Team B', [{ playerName: 'Saquon Barkley', franchiseEligible: false, previouslySaved: false }]],
    ])
  ...
  it('populates previousYearRoster by matching player names to pool IDs', () => {
    ...
    const pool: Player[] = [p]
    const roster: RosterImport = new Map([
      ['Alpha', [{ playerName: 'Travis Kelce', franchiseEligible: false, previouslySaved: false }]],
    ])
  ...
  it('seeds saveHistory from previously_saved players', () => {
    ...
    const pool: Player[] = [p]
    const roster: RosterImport = new Map([
      ['Alpha', [{ playerName: 'CeeDee Lamb', franchiseEligible: false, previouslySaved: true }]],
    ])
    const [team] = buildTeamsFromImport(roster, pool)
  ...
  it('does not seed saveHistory for non-saved players', () => {
    ...
    const pool: Player[] = [p]
    const roster: RosterImport = new Map([
      ['Alpha', [{ playerName: 'Davante Adams', franchiseEligible: false, previouslySaved: false }]],
    ])
    const [team] = buildTeamsFromImport(roster, pool)
  ...
  it('skips roster players not found in the pool (graceful)', () => {
    const pool: Player[] = []
    const roster: RosterImport = new Map([
      ['Team A', [{ playerName: 'Retired Guy', franchiseEligible: false, previouslySaved: false }]],
    ])
    const [team] = buildTeamsFromImport(roster, pool)
...
describe('autoSelectFranchise', () => {
  it('picks a franchise player for each team without one, from franchise-eligible players', () => {
    ...
    const roster: RosterImport = new Map([
      ['Team A', [{ playerName: 'Tyreek Hill', franchiseEligible: true, previouslySaved: false }]],
    ])
    const teams = buildTeamsFromImport(roster, pool)
  ...
  it('does not override a franchise player already set', () => {
    ...
    const roster: RosterImport = new Map([
      ['Team A', [
        { playerName: 'Player One', franchiseEligible: true, previouslySaved: false },
        { playerName: 'Player Two', franchiseEligible: true, previouslySaved: false },
      ]],
    ])
    const teams = buildTeamsFromImport(roster, pool)
