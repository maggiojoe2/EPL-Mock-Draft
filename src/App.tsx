...
function DraftView({ initialState }: { initialState: DraftState }) {
  ...
  const isUserTurn =
    mode === "practice" &&
    !pendingPrompt &&
    currentPick.teamIndex === userTeamIndex;
  ...
  // Transition to summary screen when the draft finishes. Adjusted during
  // render (rather than in an effect) per the React-recommended pattern for
  // deriving state from a prop change without an extra render round-trip.
  if (prevIsDraftComplete !== isDraftComplete) {
    setPrevIsDraftComplete(isDraftComplete);
  ...
  useEffect(() => {
    ...
    if (isUserTurn || isUserReactionPending) {
      simulatingRef.current = false;
      return;
    }
    ...
    <div className="app">
      ...
      {isUserReactionPending && (
        <ReactionModal
          ...
          onInvokePullback={(player) =>
            dispatch({ type: "INVOKE_PULLBACK", pullbackPlayer: player })
          }
...
const CELL_TYPE_CONFIG: Partial<
  Record<PickRecord["pickType"], { mod: string; badge: React.ReactNode }>
> = {
  franchise: {
    mod: "pick-cell--franchise",
    badge: <span className="chip-badge chip-badge--fp">★ FP</span>,
  },
  save: {
    mod: "pick-cell--save",
    badge: <span className="chip-badge chip-badge--save">💾</span>,
  },
  pullback: {
    mod: "pick-cell--pullback",
    badge: <span className="chip-badge chip-badge--pb">↩</span>,
  },
};
      ...
      <table className="draft-board">
        ...
        <tbody>
          {Array.from({ length: totalRounds }, (_, ri) => {
            const round = ri + 1;
            const isActiveRound =
              !isDraftComplete && round === currentPick.round;
            return (
      ...
      <div className="modal-overlay">
        <div className="modal">
          ...
          <p>
            ...
            <strong>{prompt.player.name}</strong> — a previous-year player on
            your roster.
          </p>
          <p>
            <strong>{reactingTeam.name}</strong>: use your one-per-draft save to
            keep them?
          </p>
          <div className="modal-actions">
            <button
              className="btn-primary"
              onClick={() => onInvokeSave(prompt.player)}
            >
  ...
  return (
    <div className="modal-overlay">
      <div className="modal">
        ...
        <p>
          <strong>{pickingTeam.name}</strong> picked{" "}
