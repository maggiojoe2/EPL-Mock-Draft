...
describe('save-or-pullback combined reaction', () => {
  function makeCombinedState() {
    ...
    const state = makeDraftState({
      teams,
      availablePool: [pickedPlayer, pullbackOption, ...Array.from({ length: 10 }, (_, i) => makePlayer(i + 1))],
  ...
  it('INVOKE_PULLBACK on a save-kind prompt pulls back the other player and leaves the original pick standing', () => {
    ...
    const afterPullback = draftEngine(afterPick, { type: 'INVOKE_PULLBACK', pullbackPlayer: pullbackOption })
  ...
  it('INVOKE_SAVE still works normally when pullbackOptions are also present', () => {
    ...
    const afterPick = draftEngine(state, { type: 'PICK_PLAYER', player: pickedPlayer })
    const afterSave = draftEngine(afterPick, { type: 'INVOKE_SAVE', player: pickedPlayer })

    expect(afterSave.teams[1]!.roster[15]).toEqual(pickedPlayer)
    expect(afterSave.teams[1]!.saveUsedThisDraft).toBe(true)
    // Picking team no longer has the player — the save blocked the pick.
    expect(afterSave.teams[0]!.roster.every(p => p?.id !== pickedPlayer.id)).toBe(true)
  })
  ...
  it('declining a combined save-or-pullback prompt still leaves save and pullback available on a later pick', () => {
    ...
    const pool = [firstPlayer, secondPlayer, pullbackOption, ...Array.from({ length: 20 }, (_, i) => makePlayer(i + 2))]
    const teams = Array.from({ length: 12 }, (_, i) =>
      i === 1 ? ownerTeam : makeTeam({ name: `Team ${i}` }),
    )

    let state = makeDraftState({ teams, availablePool: pool, currentPick: { round: 1, teamIndex: 0 } })
    ...
    state = draftEngine(state, { type: 'INVOKE_PULLBACK', pullbackPlayer: pullbackOption })
...
describe('pullback mechanics', () => {
  ...
  it('pullbackOptions are sorted ascending by ADP (best player first)', () => {
    ...
    const state = makeDraftState({
      teams,
      availablePool: [pickedPlayer, worseOption, betterOption, ...Array.from({ length: 10 }, (_, i) => makePlayer(i + 60))],
      currentPick: { round: 1, teamIndex: 0 },
    })
    ...
    if (next.pendingPrompt?.kind === 'pullback') {
      expect(next.pendingPrompt.pullbackOptions.map(p => p.id)).toEqual([betterOption.id, worseOption.id])
    }
  })
  ...
  it('does not offer a pullback when the team has no remaining eligible players in the pool', () => {
    ...
    const state = makeDraftState({
      teams,
      availablePool: [pickedPlayer, ...Array.from({ length: 10 }, (_, i) => makePlayer(i + 1))],
      currentPick: { round: 1, teamIndex: 0 },
    })
