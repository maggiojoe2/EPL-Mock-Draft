...
describe("buildCsvRows", () => {
  it("returns one row per filled slot per team", () => {
    ...
    const roster = makeRoster([p1, p2, ...emptySlots(14)]);
  ...
  it('labels franchise (round 16) slots as "franchise"', () => {
    ...
    const roster = makeRoster([...emptySlots(15), fp]);
  ...
  it('labels save slots as "save"', () => {
    ...
    const roster = makeRoster([...emptySlots(14), p, null]);
  ...
  it("omits unfilled slots", () => {
    ...
    const roster = makeRoster([p, ...emptySlots(15)]);
    const team = makeTeam("Packers", roster);

    const pickHistory: PickRecord[] = [
      { round: 1, teamIndex: 0, player: p, pickType: "normal" },
    ];
  ...
  it("handles multiple teams and assigns correct team names", () => {
    ...
    const rosterA = makeRoster([p1, ...emptySlots(15)]);
    const rosterB = makeRoster([p2, ...emptySlots(15)]);
    ...
    expect(rows).toHaveLength(2);
    expect(rows.find((r) => r.player_name === "Team A Player")?.team_name).toBe(
      "Patriots",
    );
    expect(rows.find((r) => r.player_name === "Team B Player")?.team_name).toBe(
      "Dolphins",
    );
  });
  ...
  it("orders rows by round then teamIndex", () => {
    ...
    const rosterA = makeRoster([p1, p3, ...emptySlots(14)]);
    const rosterB = makeRoster([p2, ...emptySlots(15)]);
...
describe("toCsvString", () => {
  it("produces a header row plus one data row per entry", () => {
    ...
    expect(lines[0]).toBe(
      "team_name,round,player_name,position,nfl_team,slot_type",
    );
